...
$child = null;
foreach ( $lines as $li ) { if ( $li->get_meta( '_ordelist_addon_origin' ) ) { $child = $li; } }
...
update_option( ORDELIST_Settings::OPTION, array(
	'extras_enabled' => 'yes',
...
$n2 = ORDELIST_Extras::convert( $order2 );
...
ORDELIST_Extras::convert( $order3 );
$order3 = wc_get_order( $order3->get_id() );
$child3 = null;
foreach ( $order3->get_items( 'line_item' ) as $li ) { if ( $li->get_meta( '_ordelist_addon_origin' ) ) { $child3 = $li; } }
